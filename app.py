...
if choice == "Spatial State Query":
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(f"Select a.ACC_ID, a.ACC_DESCRIPTION, a.LOC_ID, l.LOC_START_LAT, l.LOC_START_LNG, c.LOC_STATE FROM ACCIDENT a JOIN LOC l ON a.LOC_ID = l.LOC_ID JOIN COORDINATE c ON l.LOC_START_LNG = c.LOC_START_LNG WHERE c.LOC_STATE = \'{state_name}\'")
        data = cur.fetchall()
        df = pd.DataFrame(data, columns=["ACC_ID", "ACC_DESCRIPTION", "LOC_ID", "LOC_START_LAT","LOC_START_LNG", "LOC_STATE"])
        cur.close()
        conn.close()
...
if choice == "Severity Ranking Query":
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(f"Select a.ACC_ID, a.ACC_DESCRIPTION, a.TIME_ID, t.TIME_START, t.TIME_END, s.SEV_RATING FROM ACCIDENT a JOIN TIM t ON a.TIME_ID = t.TIME_ID JOIN SEVERITY s ON t.TIME_START = s.TIME_START WHERE s.SEV_RATING = {severity}")
        data = cur.fetchall()
        df = pd.DataFrame(data ,columns=["ACC_ID", "ACC_DESCRIPTION", "TIME_ID", "TIME_START","TIME_END", "SEV_RATING"])
...
if choice == "City Finder Query":
    st.write("### Find Data by City")
    city_name = st.text_input("Enter a City Name (Case Sensitive)")
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(f"Select a.ACC_ID, a.ACC_DESCRIPTION, a.LOC_ID, l.LOC_START_LAT, l.LOC_START_LNG, c.LOC_CITY FROM ACCIDENT a JOIN LOC l ON a.LOC_ID = l.LOC_ID JOIN COORDINATE c ON l.LOC_START_LNG = c.LOC_START_LNG WHERE c.LOC_CITY = \'{city_name}\'")
        data = cur.fetchall()
        df = pd.DataFrame(data,columns=["ACC_ID", "ACC_DESCRIPTION", "LOC_ID", "LOC_START_LAT","LOC_START_LNG", "LOC_CITY"])
        cur.close()
        conn.close()
...
if choice == "Time Period Query":
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute(f"Select a.ACC_ID, a.ACC_DESCRIPTION, a.TIME_ID, t.TIME_START, t.TIME_END FROM ACCIDENT a JOIN TIM t ON a.TIME_ID = t.TIME_ID WHERE (t.TIME_START > TO_DATE(\'{time_start}\', 'YYYY-MM-DD HH24:MI:SS')) AND (t.TIME_END < TO_DATE(\'{time_end}\', 'YYYY-MM-DD HH24:MI:SS'))" )
        data = cur.fetchall()
        df = pd.DataFrame(data,columns=["ACC_ID", "ACC_DESCRIPTION", "TIME_ID", "TIME_START","TIME_END"])
        cur.close()
        conn.close()
